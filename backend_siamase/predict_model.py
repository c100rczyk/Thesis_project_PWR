...
image_size = 224, 224
margin = 0.5
representatives_path = r"data/represent_5"


def load_representatives(representatives_path):
    """
    ...
    """
    products_paths = DataReader.read_types_of_products(representatives_path)
...
def create_contrastive_model():
    """
    Choose architecture and load previously trained weights to model.
    :return: Embedding network for creating embedding for image
    """
    from keras.api.applications.densenet import DenseNet201
    modelNet = DenseNet201(include_top=False, weights='imagenet')

    input_layer = keras.layers.Input(shape=(224, 224, 3))
    x = modelNet(input_layer)

    x = keras.layers.Flatten()(x)
    x = keras.layers.Dense(64, activation='relu')(x)
    x = keras.layers.Lambda(lambda x: tf.math.l2_normalize(x, axis=-1))(x)
    embedding_network = keras.Model(input_layer, outputs=x)
    ...
    merge_layer_distance = keras.layers.Lambda(EuclideanDistance(), output_shape=(1,))([embed_1, embed_2])
    siamase = keras.Model(inputs=[input_1, input_2], outputs=merge_layer_distance)

    for layer in modelNet.layers:
        layer.trainable = False
    siamase.load_weights(r"../trained_models/Net201_VegFru_CL_10epok_50tys_5tys_64embed_top5_90_RruitRecognition.weights.h5")

    return embedding_network, siamase
